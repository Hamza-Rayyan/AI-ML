...
sns.set(style="whitegrid") # Default seaborn look and feel
...
sns.countplot(x='Gender',hue='Married',palette='Set1',data=df)
print(pd.crosstab(df['Gender'],df['Married']))
plt.savefig('Gender vs Married')
plt.show()

sns.countplot(x='Gender',hue='Self_Employed',palette='Set1',data=df)
print(pd.crosstab(df['Gender'],df['Self_Employed']))
plt.savefig('Gender vs Self Employed')
plt.show()

sns.countplot(x='Gender',hue='Property_Area',palette='Set1',data=df)
...
#apply this to an algo for slpitting our data randomly
seed = 42

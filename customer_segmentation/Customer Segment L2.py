...
sns.countplot(x = 'Gender',hue='Ever_Married',palette='Set1', data = df)
print(pd.crosstab(df['Gender'],df['Ever_Married']))
plt.savefig('Gender vs Ever_Married')
plt.show()

sns.countplot(x = 'Gender',hue='Profession',palette='Set1', data = df)
print(pd.crosstab(df['Gender'],df['Profession']))
plt.savefig('Gender vs Profession')
plt.show()

sns.countplot(x = 'Gender', hue = 'Spending_Score', palette = 'Set1', data= df)
print(pd.crosstab(df['Gender'], df['Spending_Score']))
plt.savefig('Gender vs Spending_Score')
plt.show


sns.countplot(x = 'Graduated',hue= 'Profession',palette='Set1', data = df)
print(pd.crosstab(df['Graduated'],df['Profession']))
plt.savefig('Graduated vs Profession')
plt.show()


# in my analysis graduated wont bring any better insights so we may delete the graduate column.
del df['Graduated']

# Converting categorical data into numeriacal data
